...
public class OrderNumberUtils {
    ...
    // 시퀀스 번호를 위한 비트 수. 비트는 컴퓨터에서 데이터를 표현하는 최소 단위입니다.
    private static final long SEQUENCE_BIT = 12;
    // 시퀀스 번호의 최대값. 이 값을 넘으면 다시 0으로 초기화됩니다.
    private static final long MAX_SEQUENCE = ~(-1L << SEQUENCE_BIT);

    // 워커 ID와 데이터 센터 ID를 위한 비트 수. 워커는 서버나 애플리케이션 인스턴스를 의미합니다.
    ...
    // 워커 ID(서버나 애플리케이션 인스턴스의 고유 번호)와 데이터 센터 ID(데이터 센터의 고유 번호)
    @Value("${worker.id}")
    private long workerId;

    // 마지막으로 주문번호를 생성한 시간
    private long lastTimestamp = -1L;
    // 시퀀스 번호. 주문번호 생성 시 사용되는 일련번호입니다.
    private long sequence = 0L;

    /**
     * 주문 번호를 생성하는 메서드.
     * 주문 번호는 현재 시간, 워커 ID, 데이터 센터 ID, 시퀀스 번호를 조합하여 생성됩니다.
     *
    ...
    public synchronized String generateOrderNumber() {
        ...
        // 현재 시간, 워커 ID, 데이터 센터 ID, 시퀀스 번호를 조합하여 주문 번호를 생성합니다.
     ...
     * 주문 번호 생성 시 같은 밀리초 내에서 여러 번호를 생성할 경우 사용됩니다.
     *
